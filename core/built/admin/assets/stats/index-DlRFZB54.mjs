import { a4 as U, W, a0 as ae, a3 as se, M as F, $ as m, y as D, V as re, H as e, z as S, w as ne, q as xe, N as ge, R as q } from "./index-D39xCs8v.mjs";
import { by as fe, g as te, N as be, Q as je, O as ve, P as Y, Y as ke, s as we, o as Ne, aU as ye, aV as Se, aY as _e, w as Ce, ao as Te, B as De, S as le, a8 as ie, bg as J, at as Re, ay as Le, aa as Me, az as Pe, au as Oe, av as Be, ax as Ee, aw as Ae, bh as X, bb as K, bN as ze, bI as Ve, bJ as He, aG as $e, aF as Ke, ag as We, K as Fe, aH as Ie, i as ce, j as oe, m as Ge, n as Ye, k as Qe, aE as Ue, h as qe } from "./stats-BSUAFETq.mjs";
import { T as Je, c as Xe, a as Q, b as O, K as Ze } from "./kpi-tabs-CxFjWPUJ.mjs";
import { B as et, T as tt, a as at, e as st, f as $, d as E, S as H, b as L } from "./sort-button-DCiFy3OX.mjs";
const rt = ({ active: s, payload: t }) => {
  ...
  const [l, h] = U(i), [p, v] = U(!1), { range: u } = W(), w = ae(), [b] = se(), { appSettings: N } = F(), { emailTrackClicks: j, emailTrackOpens: y } = N?.analytics || {}, { totalSubscribers: M, avgOpenRate: d, avgClickRate: o } = r, f = m(() => {
    if (!s || s.length === 0)
      return [];
    let c = [];
  ...
  return /* @__PURE__ */ e.jsxs(Je, { defaultValue: i, variant: "kpis", children: [
    /* @__PURE__ */ e.jsxs(Xe, { className: `-mx-6 hidden grid-cols-3 md:visible! md:grid! ${I}`, children: [
  ...
  const { selectedNewsletterId: t, setSelectedNewsletterId: r } = W(), n = m(() => s?.filter((a) => a.status === "active") || [], [s]);
  return re(() => {
    if (n.length > 0 && !t) {
  ...
  const n = s ?? 30, { startDate: a, endDate: i } = m(() => X(n), [n]), l = m(() => {
    const p = {
      date_from: K(a),
      date_to: K(i)
    };
  ...
  const a = s ?? 30, i = t ?? "date desc", { startDate: l, endDate: h } = m(() => X(a), [a]), p = m(() => {
    const u = {
      date_from: K(l),
      date_to: K(h),
      order: i
...
}, ue = q.memo(({ range: s, selectedNewsletterId: t, shouldFetchStats: r, sortBy: n }) => {
  const a = ae(), { settings: i } = W(), l = String(i.find((d) => d.key === "timezone")?.value || "Etc/UTC"), { data: h, isLoading: p, isClicksLoading: v } = de(
    s,
    n,
    // Reactive to sort changes, but only affects this component
    t || void 0,
    !!r
  ), { appSettings: u } = F(), { emailTrackClicks: w, emailTrackOpens: b } = u?.analytics || {}, N = m(() => h?.stats || [], [h]), j = b && w ? 5 : b || w ? 4 : 3, y = m(() => /* @__PURE__ */ e.jsx(e.Fragment, { children: /* @__PURE__ */ e.jsx($, { className: "last:border-none [&>td]:py-2.5", children: /* @__PURE__ */ e.jsx(L, { className: "font-medium", colSpan: j, children: /* @__PURE__ */ e.jsx(Ue, { className: "mt-5" }) }) }) }), [j]), M = m(() => N.length > 0 ? /* @__PURE__ */ e.jsx(e.Fragment, { children: N.map((d) => /* @__PURE__ */ e.jsxs($, { className: "last:border-none [&>td]:py-2.5", children: [
    /* @__PURE__ */ e.jsx(L, { className: "font-medium", children: /* @__PURE__ */ e.jsx("div", { className: "group/link inline-flex items-center gap-2", children: d.post_id ? /* @__PURE__ */ e.jsx(qe, { className: "h-auto p-0 text-left whitespace-normal hover:underline!", title: "View post analytics", variant: "link", onClick: () => {
  ...
  ] }, d.post_id)) }) : /* @__PURE__ */ e.jsx($, { className: "border-none hover:bg-transparent", children: /* @__PURE__ */ e.jsx(L, { className: "text-center group-hover:bg-transparent!", colSpan: j, children: /* @__PURE__ */ e.jsx(
    le,
    {
      className: "size-full py-20",
      title: `No newsletters ${J(s)}`,
      children: /* @__PURE__ */ e.jsx(ie, { strokeWidth: 1.5 })
    }
  ) }) }), [N, v, a, w, b, s]);
  return p || !h ? y : M;
...
const me = q.memo(({ sortBy: s, setSortBy: t, range: r }) => {
  ...
  return /* @__PURE__ */ e.jsx(st, { children: /* @__PURE__ */ e.jsxs($, { children: [
...
const pe = q.memo(({ range: s, selectedNewsletterId: t, shouldFetchStats: r }) => {
...
const gt = () => {
  const { range: s, selectedNewsletterId: t } = W(), [r] = se(), { appSettings: n } = F(), a = r.get("tab") || "total-subscribers", { data: i, isLoading: l } = ct({
    searchParams: {
      limit: "50"
    }
    ...
    if (u?.stats && u.stats.length > 0) {
      const x = u.stats, C = x.reduce((T, P) => T + (P.open_rate || 0), 0), g = x.reduce((T, P) => T + (P.click_rate || 0), 0);
    ...
    if (!p?.stats?.[0]?.values || p.stats[0].values.length === 0) {
      const { startDate: f, endDate: _ } = X(s), x = [], C = f.clone().toDate(), g = _.toDate();
      for (; C <= g; )
        x.push({
  ...
  return n && !n.newslettersEnabled ? /* @__PURE__ */ e.jsx(ge, { to: "/analytics" }) : /* @__PURE__ */ e.jsxs($e, { children: [
...
//# sourceMappingURL=index-DlRFZB54.mjs.map
