import { T as st, Q, O as v, af as it, P as Ee, bK as Ie, ak as H, bD as ot, bN as m, bM as F, h as P, aC as z, t as fe, V as at, bw as lt, j as ht, r as ft, bj as ut, aX as dt, aP as pt, e as ct, i as mt, aW as Me, bi as gt, bx as kt, s as ue, S as Lt } from "./index-SJHuYNt9.mjs";
import { html as bt, htmlCompletionSource as wt } from "./index-GdNwjcF8.mjs";
class U {
...
function se(t, e, r) {
  ...
  return r && t.next == 45 && Oe(t) > -1 && t.depth == e.stack.length && e.parser.leafBlockParsers.indexOf(De.SetextHeading) > -1 || n < 3 ? -1 : 1;
}
function Ne(t, e) {
...
const $ = {
  ...
  HTMLBlock(t, e) {
    ...
    o && t.nextLine();
    let a = s == Re ? u.CommentBlock : s == Te ? u.ProcessingInstructionBlock : u.HTMLBlock, l = t.prevLineEnd();
  ...
  advance(e) {
    for (; ; ) {
      ...
      if (this.stage == 0) {
        ...
        if (!this.nextStage(qe(e, R(e, this.pos), this.start)))
          return -1;
...
class xt {
  nextLine(e, r, n) {
    let s = r.depth < e.stack.length ? -1 : Oe(r), i = r.next;
    if (s < 0)
      return !1;
...
function Xe(t, e, r, n, s) {
  ...
  for (let f = r.firstChild; f; f = f.nextSibling) {
    ...
    c ? d = c : f.to + n > i ? (d = Xe(t, e, f, n, s), h(f.to + n, !1)) : d = f.toTree(), o.push(d), a.push(p - l);
  }
...
class Z extends st {
  ...
  */
  configure(e) {
    ...
    if (O(r.props) && (n = n.extend(...r.props)), O(r.remove))
...
const G = {
  ...
  Emphasis(t, e, r) {
    ...
    for (; t.char(n) == e; )
      n++;
    let s = t.slice(r - 1, r), i = t.slice(n, n + 1), o = X.test(s), a = X.test(i), l = /\s|^$/.test(s), h = /\s|^$/.test(i), f = !h && (!a || l || o), p = !l && (!o || h || a), d = f && (e == 42 || !p || o), c = p && (e == 42 || !f || a);
...
const vt = Ie({
  ...
  LinkTitle: m.string,
  Paragraph: m.content
}), Nt = new Z(new Ee(Fe).extend(vt), Object.keys($).map((t) => $[t]), Object.keys($).map((t) => De[t]), Object.keys($), Bt, He, Object.keys(G).map((t) => G[t]), Object.keys(G), []);
function yt(t, e, r) {
  let n = [];
...
const Rt = { resolve: "Strikethrough", mark: "StrikethroughMark" }, Tt = {
  defineNodes: [{
    name: "Strikethrough",
  ...
  parseInline: [{
    ...
    parse(t, e, r) {
      if (e != 126 || t.char(r + 1) != 126 || t.char(r + 2) == 126)
        return -1;
      let n = t.slice(r - 1, r), s = t.slice(r + 2, r + 3), i = /\s|^$/.test(n), o = /\s|^$/.test(s), a = X.test(n), l = X.test(s);
...
}, Le = /(www\.)|(https?:\/\/)|([\w.+-]{1,100}@)|(mailto:|xmpp:)/gy, be = /[\w-]+(\.[\w-]+)+(\/[^\s<]*)?/gy, Ft = /[\w-]+\.[\w-]+($|\/)/, we = /[\w.+-]+@[\w-]+(\.[\w.-]+)+/gy, Se = /\/[a-zA-Z\d@.]+/gy;
function Ce(t, e, r, n) {
  let s = 0;
  for (let i = e; i < r; i++)
    t[i] == n && s++;
...
const Gt = /* @__PURE__ */ dt.of((t, e, r) => {
  for (let n = F(t).resolveInner(r, -1); n && !(n.from < e); n = n.parent) {
...
const Wt = (t = {}) => ({ state: e, dispatch: r }) => {
  let n = F(e), { doc: s } = e, i = null, o = e.changeByRange((a) => {
...
const nr = ({ state: t, dispatch: e }) => {
  let r = F(t), n = null, s = t.changeByRange((i) => {
...
function ur(t = {}) {
  ...
  l.push(Ot({ codeParser: p, htmlParser: a.language.parser })), n && h.push(at.high(lt.of(sr)));
...
//# sourceMappingURL=index-COSRYfwI.mjs.map
